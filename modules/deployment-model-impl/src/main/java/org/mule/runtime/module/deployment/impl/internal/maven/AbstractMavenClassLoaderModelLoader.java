...
public abstract class AbstractMavenClassLoaderModelLoader implements ClassLoaderModelLoader {
  ...
  private ClassLoaderModel createLightPackageClassLoaderModel(File artifactFile,
    ...
    // This is already filtering out mule-plugin dependencies,
    // since for this case we explicitly need to consume the exported API from the plugin.
    final List<URL> dependenciesArtifactsUrls = loadUrls(artifactFile, classLoaderModelBuilder, concat(nonProvidedDependencies.stream(),
                                                                 missingApiDependencyBundles.stream()).collect(toSet()), emptyList());
