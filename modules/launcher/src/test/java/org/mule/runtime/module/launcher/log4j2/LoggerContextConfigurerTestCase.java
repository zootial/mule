...
public class LoggerContextConfigurerTestCase extends AbstractMuleTestCase {
  ...
  public void forceConsoleLogWithAppenderAlreadyPresent() {
    withForceConsoleLog(() -> {
      ...
      appenders.add(ConsoleAppender.newBuilder().setName("Console").setLayout(mock(Layout.class)).build());
  ...
  private void withForceConsoleLog(Runnable assertion) {
    final String originalValue = getProperty(MULE_FORCE_CONSOLE_LOG);

    setProperty(MULE_FORCE_CONSOLE_LOG, "");
    try {
      assertion.run();
    } finally {
      if (originalValue != null) {
        setProperty(MULE_FORCE_CONSOLE_LOG, originalValue);
      } else {
        clearProperty(MULE_FORCE_CONSOLE_LOG);
      }
    }
  }
}
